...
module Spirograph.Shapes {
    export function RingGear(options: RingGearOptions) {
        ...
        for (var i = 0; i < options.outerToothCount; i++) {
            ...
            pathBuilder.add(new SVG.ACommand(options.outerRadius, options.outerRadius, 0, false, true, options.outerRadius * Math.cos(Utility.toRadians(angle + delta * 4 / 6)), options.outerRadius * Math.sin(Utility.toRadians(angle + delta * 4 / 6))));
        ...
        for (var i = 0; i < options.innerToothCount; i++) {
            pathBuilder.add(new SVG.ACommand(options.innerRadius, options.innerRadius, 0, false, true, options.innerRadius * Math.cos(Utility.toRadians(angle + delta * 1 / 6)), options.innerRadius * Math.sin(Utility.toRadians(angle + delta * 1 / 6))));
